...
def generate_launch_description():
    ...
    two_wheel_robot = get_package_share_directory('two_wheel_robot')

    gazebo_models_path, ignore_last_dir = os.path.split(two_wheel_robot)
    ...
    model_arg = DeclareLaunchArgument(
        'model', default_value='two_wheel_robot.xacro',
    ...
    urdf_file_path = PathJoinSubstitution([
        two_wheel_robot,  # Replace with your package name
        "urdf",
        LaunchConfiguration('model')  # Replace with your URDF or Xacro file
    ])

    gz_bridge_params_path = os.path.join(
        get_package_share_directory('two_wheel_robot'),
        'config',
        'gz_bridge.yaml'
    )
    ...
    world_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(two_wheel_robot, 'launch', 'world.launch.py'),
        ),
        launch_arguments={
    ...
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', PathJoinSubstitution([two_wheel_robot, 'rviz', LaunchConfiguration('rviz_config')])],
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(two_wheel_robot, 'config', 'ekf.yaml'),
            {'use_sim_time': LaunchConfiguration('use_sim_time')},
             ]
    )
