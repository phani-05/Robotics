...
def generate_launch_description():
    ...
    gazebo_models_path, ignore_last_dir = os.path.split(pkg_amr)
    ...
    model_arg = DeclareLaunchArgument(
        'model', default_value='amr_st.urdf',
    ...
    urdf_file_path = PathJoinSubstitution([
        pkg_amr,  # Replace with your package name
        "urdf",
    ...
    world_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_amr, 'launch', 'world.launch.py'),
        ),
        launch_arguments={
    ...
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', PathJoinSubstitution([pkg_amr, 'rviz', LaunchConfiguration('rviz_config')])],
    ...
    trajectory_node = Node(
        ...
        parameters=[{'reference_frame_id': 'base_link'}]
    )
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(pkg_amr, 'config', 'ekf.yaml'),
            {'use_sim_time': LaunchConfiguration('use_sim_time')},
             ]
    )
